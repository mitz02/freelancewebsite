...
// Function to send a file (you can use this function when integrating with the backend)
function sendFile(file, sender) {
    const reader = new FileReader();
    reader.onload = function(event) {
        const fileData = event.target.result;

        // Use jQuery AJAX to send the file to the backend
        $.ajax({
            type: "POST",
            url: "backend.php",
            data: { file: fileData, sender: sender },
            success: function(response) {
                console.log(response);
            },
            error: function(error) {
...
function appendMessage(sender, content, messageType) {
    ...
    const messageDiv = $("<div>").addClass("message").addClass(messageType);
    const messageContent = $("<p>").text(content);
    const timestampSpan = $("<span>").addClass("timestamp").text(timestamp);

    messageDiv.append(messageContent).append(timestampSpan);
...
// Function to get a formatted timestamp (HH:mm)
